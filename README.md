...
- **전부 부드러운 글꼴 (안티에일리어스)** — 시간은 대형 나눔고딕 Bold(약 72px, `src/timefont.h`), 날짜·날씨·습도·오전/오후는 나눔고딕 Regular(`src/labelfont.h`)로 픽셀 없이 매끄럽게 표시 (나눔고딕은 OFL 오픈 폰트)
- **지역 표기** — 광역시/도 + 도시명(예: "서울특별시 서울")까지 표시. 습도는 물방울 아이콘과 함께.
...
```
python -m esptool --chip esp32 merge_bin -o docs/firmware-v1.6.0.bin ^
